...
move_uploaded_file($_FILES['image']['tmp_name'],"files/".time()."_".$_FILES['image']['name']);
} */
if(isset($_POST['upload'])){
$f = $_FILES['image'];
...
if(in_array($f['type'],$allowed)&& $f['size'] <= 1000000){
    $name = rand(10000,99999)."_".time().".jpg";
    move_uploaded_file($f['tmp_name'],"images/".$name);
    $message = "upload complete";
}
else if(in_array($f['type'],$allowedpdf)){
